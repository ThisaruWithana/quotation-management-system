            ...
            <tr style="page-break-inside: avoid;">
                ...
                <td style="width: 33.33%; padding: 10px; vertical-align: top;">
                    <table style="width: 200px; border-collapse: collapse; page-break-inside: avoid;max-width: 200px">
                        <tr>
                            <td colspan="2" style="padding: 8px; text-align: left; font-weight: bold; font-size:10px;">{{ $value['name'] }}</td>
                        </tr>
                        <tr>
                            <td style="text-align: left; width: 50%; font-size:10px;">{{ $value['item_size'] }}</td>
                            <td style="text-align: right; width: 50%; font-size:10px;">{{ number_format($value['retail_price'], 2) }}</td>
                        </tr>
                        <tr>
                            <td colspan="2" style="padding: 8px; text-align: center; font-size:10px;">
                                {!! DNS1D::getBarcodeHTML($value['barcode']['barcode'], 'EAN13') !!}
                            </td>
                        </tr>
                        <tr>
                            <td colspan="2" style="text-align: center; font-size:10px;">{{ $value['barcode']['barcode'] }}</td>
                        </tr>
                    </table>
                </td>
                @if(($index + 1) % 3 == 0)
